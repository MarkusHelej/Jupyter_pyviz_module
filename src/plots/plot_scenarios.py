...
def _style_ridge_panel(ax: plt.Axes, *, xleft: float | None = None, show_ylabel: bool = False) -> None:
    ...
    # Keep only bottom ticks; y ticks/labels look cleaner off
...
def _ridgeline_axes(
    ...
    *,
    ...
    if value_col == "beam_p_s" and log_x:
        ...
        ax.set_xlim(left=max(cur_left, 1e-12), right=1e15)  # <- force right to 1e15
        ax.margins(x=0.01)
...
def plot_scenarios(
    ...
    *,
    ...
    """
    Build figures mirroring the R layout:
      fig_1 = plot00 / plot02 / plot01
      fig_2 = plot1  / plot2
    Returns: (fig_1, fig_2)
    """
